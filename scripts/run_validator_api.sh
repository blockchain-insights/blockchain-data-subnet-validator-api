...
# Check if the api_key.json file exists and create a symlink if it does
if [ -f /app/config/api_key.json ]; then
  ln -s /app/config/api_key.json /app/api_key.json
fi

# Check if the rate_limit.json file exists and create a symlink if it does
if [ -f /app/config/rate_limit.json ]; then
  ln -s /app/config/rate_limit.json /app/rate_limit.json
fi
